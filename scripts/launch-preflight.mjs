...
if (!existsSync(indexPath)) {
  ...
  if (localBuild >= 115) pass(`local build tag (build ${localBuild})`);
  else failMsg("local build tag", `expected build 115+, got ${localBuild || "none"}`);
...
if (!existsSync(wwwIdx)) {
  ...
  if (wwwBuild >= 115) pass(`www build tag (build ${wwwBuild})`);
  else failMsg("www build tag", `expected build 115+, got ${wwwBuild || "none"}`);
