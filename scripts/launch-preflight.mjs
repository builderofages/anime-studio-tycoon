...
if (!existsSync(indexPath)) {
  ...
  if (localBuild >= 110) pass(`local build tag (build ${localBuild})`);
  else failMsg("local build tag", `expected build 110+, got ${localBuild || "none"}`);
...
if (!existsSync(wwwIdx)) {
  ...
  if (wwwBuild >= 110) pass(`www build tag (build ${wwwBuild})`);
  else failMsg("www build tag", `expected build 110+, got ${wwwBuild || "none"}`);
