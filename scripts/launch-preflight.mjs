...
if (!existsSync(indexPath)) {
  ...
  if (localBuild >= 113) pass(`local build tag (build ${localBuild})`);
  else failMsg("local build tag", `expected build 113+, got ${localBuild || "none"}`);
...
if (!existsSync(wwwIdx)) {
  ...
  if (wwwBuild >= 113) pass(`www build tag (build ${wwwBuild})`);
  else failMsg("www build tag", `expected build 113+, got ${wwwBuild || "none"}`);
