...
if (!existsSync(indexPath)) {
  ...
  if (localBuild >= 111) pass(`local build tag (build ${localBuild})`);
  else failMsg("local build tag", `expected build 111+, got ${localBuild || "none"}`);
...
if (!existsSync(wwwIdx)) {
  ...
  if (wwwBuild >= 111) pass(`www build tag (build ${wwwBuild})`);
  else failMsg("www build tag", `expected build 111+, got ${wwwBuild || "none"}`);
