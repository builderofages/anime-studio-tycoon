...
if (!existsSync(indexPath)) {
  ...
  if (localBuild >= 112) pass(`local build tag (build ${localBuild})`);
  else failMsg("local build tag", `expected build 112+, got ${localBuild || "none"}`);
...
if (!existsSync(wwwIdx)) {
  ...
  if (wwwBuild >= 112) pass(`www build tag (build ${wwwBuild})`);
  else failMsg("www build tag", `expected build 112+, got ${wwwBuild || "none"}`);
