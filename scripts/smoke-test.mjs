...
assert(html.includes("build 118"), "build 118 tag");
assert(html.includes("Production Score"), "production score label");
assert(html.includes("Complete"), "build 118 complete tag");
...
assert(html.includes("Build 118"), "what's new build 118");
assert(html.includes("Build 118 — Complete"), "what's new complete headline");
assert(html.includes('t("prem_first_ribbon")'), "premiere modal i18n wired");
assert(html.includes('tf("hub_welcome"'), "return hub i18n wired");
assert(readFileSync(join(root, "strings.js"), "utf8").includes("const UI5"), "UI5 i18n batch");
assert(readFileSync(join(root, "strings.js"), "utf8").includes("const UI6"), "UI6 hub i18n batch");
assert(readFileSync(join(root, "strings.js"), "utf8").includes("hub_welcome"), "hub i18n keys");
assert(readFileSync(join(root, "hud-premium.js"), "utf8").includes("coach_rank_progress"), "coach pathway i18n keys");
assert(readFileSync(join(root, "hud-premium.js"), "utf8").includes("drawerSlotSignature"), "drawer slot signature guard");
...
assert(html.includes("bgm.volume=shouldPlayBgm") || html.includes("bgm.volume = shouldPlayBgm"), "music vol applies to active bgm");
assert(readFileSync(join(root, "hud-premium.js"), "utf8").includes("drawer-game-prefs"), "drawer gameplay toggles");
assert(readFileSync(join(root, "hud-premium.js"), "utf8").includes("syncDrawerAudio();\n    _drawerPrevFocus"), "openDrawer syncs drawer state");
assert(readFileSync(join(root, "hud-premium.css"), "utf8").includes("max-height: min(calc(100dvh"), "settings drawer scrolls on mobile");
assert(!readFileSync(join(root, "ast-v5.css"), "utf8").includes('content: " · Guided"'), "no duplicate guided css suffix");
...
assert(html.includes("Auto-greenlight") || html.includes("auto-greenlight"), "what's new drawer gameplay bullet");
assert(html.includes("second tap") || html.includes("unlockAudioFromGesture"), "what's new play audio unlock bullet");
...
assert(readFileSync(join(root, "hud-premium.js"), "utf8").includes("coach_research_trend"), "coach trending research tip i18n");
